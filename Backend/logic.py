...
class PlanGenerator:
    ...
    def _generate_meals_with_proper_rotation(self, all_foods, fruits, dry_fruits, vegetables, dairy, non_veg,
        ...
        for day in range(7):
            ...
            # Get items for this day (different from other days)
            ...
            if diet_pref == "veg":
                ...
                # Store daily items for this day
                day_items = {
                    "veg": veg_for_day,
                    "dairy": dairy_for_day,
                    "fruit": fruit_for_day,
            ...
            # Each meal gets one item from the daily combo
            meal_items = [
                {"meal": "breakfast", "item": dairy_for_day},  # Breakfast gets Dairy
                {"meal": "lunch", "item": fruit_for_day if diet_pref == "veg" else non_veg_for_day},  # Lunch gets Fruit(for veg) or Non-Veg(for non-veg)
                {"meal": "dinner", "item": veg_for_day if diet_pref == "veg" else fruit_for_day},  # Dinner gets Veg(for veg) or Fruit(for non-veg)
                {"meal": "snacks", "item": dry_fruit_for_day}  # Snacks gets Dry Fruit
            ]
            
            # Adjust if any item is missing
            available_meals = [m for m in meal_items if m["item"] is not None]
            ...
            for meal in meal_types:
                ...
                # Fallback if still no food
                if not selected_food and all_foods:
                    fallback = [f for f in all_foods if f.category in ["fruits", "dry_fruits"]]
                    if fallback:
                        selected_food = random.choice(fallback)
    ...
    def _generate_grocery_from_db(self, all_foods, fruits, dry_fruits, vegetables, dairy, non_veg,
        ...
        if diet_pref == "veg":
            ...
            categories = [
                (veg_list, 7, "500g", "🥬 Vegetables"),  # 7 days = 7 different veggies
                (dairy_list, 7, "500g", "🥛 Dairy"),     # 7 days = 7 different dairy
                (fruits_list, 7, "500g", "🍎 Fruits"),   # 7 days = 7 different fruits
                (dry_fruits_list, 7, "250g", "🥜 Dry Fruits")  # 7 days = 7 different dry fruits
            ]
        else:
            ...
            categories = [
                (non_veg_list, 7, "1 kg", "🍗 Non-Veg"),   # 7 days = 7 different non-veg items
                (dairy_list, 7, "500g", "🥛 Dairy"),       # 7 days = 7 different dairy
                (fruits_list, 7, "500g", "🍎 Fruits"),     # 7 days = 7 different fruits
                (dry_fruits_list, 7, "250g", "🥜 Dry Fruits")  # 7 days = 7 different dry fruits
            ]
        ...
        for cat_foods, num_items, quantity, label in categories:
            ...
            if available:
                # Take enough items for all 7 days
                ...
                for food in selected:
                    ...
                    cost = min(food_price, remaining_budget * 0.15)  # Reduced to 15% to fit all items
        ...
        if total > weekly_budget and total > 0:
            factor = weekly_budget / total
            for item in grocery_list:
                item["cost"] = round(item["cost"] * factor, 2)
        
        print(f"🛒 Grocery list total: ₹{sum(item['cost'] for item in grocery_list)} (Budget: ₹{weekly_budget})")
